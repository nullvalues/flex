...
## L017 — External CER on forqsite 2026-05-18: agent declined DL.44 (tenant-scoped page editor) as a 'hypothetical multi-tenant' future concern when in fact it was a documented spec non-negotiable AND the first customer is multi-tenant from day one. Additionally, agent asserted gaps in the codebase (RBAC management UI, tenant-scoped page editing routes) that were already specified and built — phase-rbac-ui.md / cp-rbac-ui already shipped the matrix UI; tenant editor routes existed at /admin/[orgSlug]/[tenantSlug]/pages/. Operator caught both errors and required a session-long restructure of the CER agreements.
**Date:** 2026-05-19
**Status:** captured
**Learning:** Two compounding sources of triage error. First: every declined item or asserted gap MUST be cross-referenced against (a) spec non-negotiables in the project's structured spec authority, and (b) existing phase docs that may have already specified or shipped the work. Second: when running a point-by-point agreement protocol, engage tightly — one question per turn, no multi-section responses, no precooked counter-arguments. Comprehensive responses make the operator do too much reading and miss errors. The CORA-style protocol in docs/agreements/ exists precisely to prevent this; the agent must follow it strictly when invoked.
