...
**Build standards** (per-project facts the builder/reviewer procedure skills read from here instead of hardcoding, INFRA-240): test_command=`PATH=$HOME/.local/bin:$PATH uv run pytest tests/pairmode/ -x -q` | test_dir=`tests/pairmode/` | protected_paths=`hooks/, skills/seed/scripts/, skills/companion/scripts/sidebar.py, .claude-plugin/plugin.json, .claude-plugin/marketplace.json, lessons/lessons.json` | domain_isolation_rule=`(none) — flex is a single-operator tool, not multi-tenant` | covered_contracts=`## Pairmode build loop::skills/pairmode/scripts/cer.py, ## Module structure::skills/pairmode/scripts/next_action.py` — the two seed pairs (INFRA-317): the CER backlog row format/parsing grammar (`cer.is_resolution_marked`, `find_open_do_now_rows`) is documented in `docs/architecture.md` § Pairmode build loop, and the next-action JSON grammar (`make_action`, `ACTIONS`, `validate_action`) is documented in `docs/architecture.md` § Module structure; a builder touching either source file must read both halves per `builder/procedure.md`'s covered-contracts gate.
