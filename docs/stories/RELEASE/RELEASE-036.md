...
id: RELEASE-036
rail: RELEASE
title: Fleet migration — sync pokus to pairmode 0.3.0 thin-harness loop
status: planned
...
- `/mnt/work/pokus` is on pairmode 0.2.0 with `binding: version`
...
- **Cross-repo scope, not enforced by scope_guard**: this story's actual file
  writes happen entirely inside `/mnt/work/pokus`, outside
...
- Seam gate (`docs/harness-cutover-runbook.md` § Seam gate) passes for
  `/mnt/work/pokus`:
  - `git -C /mnt/work/pokus status --porcelain` is empty (all work committed
    and pushed).
  - `uv run python flex_build.py read-attempt-count` (run from
    `/mnt/work/pokus`) returns `0`.
  - `/mnt/work/pokus` is at a phase boundary (checkpoint tag, or every
    current-phase story `status: complete`).
  - `flex_build.py check-stubs --project-dir /mnt/work/pokus` returns clean.
  - `/mnt/work/pokus` appears in `fleet_discovery.py list-projects`.
  If any seam-gate item fails, defer this story until `/mnt/work/pokus`'s
  own maintainer resolves it — do not force migration mid-story.

## Ensures

- `/mnt/work/pokus`'s `.companion/state.json` reads
  `"pairmode_version": "0.3.0"`.
- `/mnt/work/pokus`'s `CLAUDE.build.md` bakes an absolute
  `pairmode_scripts_dir` pointing at
  `/mnt/work/flex-harness/skills/pairmode/scripts` (Signal-1 present).
- `fleet_discovery.py discover --project-dir /mnt/work/pokus` (run from
  flex-harness) reports `binding: scripts`.
- One complete pairmode story cycle has run through `/mnt/work/pokus`'s
  migrated loop with no regressions, and the result is committed and pushed
  in `/mnt/work/pokus`'s own repo.
...
§ Per-project mechanic) against `/mnt/work/pokus`, all commands run with
`PATH=$HOME/.local/bin:$PATH` from `/mnt/work/flex-harness` except where
noted:

1. Confirm the seam gate (Requires) passes.
2. Dry-run:
   `uv run python skills/pairmode/scripts/pairmode_sync.py sync-all --project-dir /mnt/work/pokus --dry-run`.
   Review the diff.
3. Apply:
   `uv run python skills/pairmode/scripts/pairmode_sync.py sync-all --project-dir /mnt/work/pokus --apply --yes`.
4. Normalize:
   `uv run python skills/pairmode/scripts/pairmode_migrate.py to-030 --project-dir /mnt/work/pokus --apply`.
5. Verify Signal-1:
   `uv run python skills/pairmode/scripts/fleet_discovery.py discover --project-dir /mnt/work/pokus`
   shows `binding: scripts`. If absent, stop and fix `CLAUDE.build.md` before
   continuing.
6. Run one complete story cycle in `/mnt/work/pokus`'s migrated loop (any
   story — doc/lesson stories are fine). Confirm `pairmode_version: 0.3.0`
   in `.companion/state.json`, then from `/mnt/work/pokus`:
...
(`git -C /mnt/work/pokus checkout HEAD -- CLAUDE.build.md .companion/state.json`)
and re-run the Era 2 loop; do not leave `/mnt/work/pokus` half-migrated.

## Tests

- `PATH=$HOME/.local/bin:$PATH uv run python skills/pairmode/scripts/fleet_discovery.py discover --project-dir /mnt/work/pokus`
  reports `pairmode_version: 0.3.0` and `binding: scripts`.
- `git -C /mnt/work/pokus status --porcelain` is empty after the story
  (committed and pushed).
- If `/mnt/work/pokus` has its own test suite, it passes post-migration
