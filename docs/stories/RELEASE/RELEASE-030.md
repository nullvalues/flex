...
id: RELEASE-030
rail: RELEASE
title: Fleet migration — sync Repo-G to pairmode 0.3.0 thin-harness loop
status: backlog
...
`Repo-G` was the testbed where anchor/flex build-loop principles were
...
version of this rule at the global level, but Repo-G's specific artifacts
...
no further development). Repo-G combines the 0.1.0 schema gap (below) with
...
is updated to exclude `Repo-G` from the current fold's hard-block
...
- `/mnt/work/Repo-G` is on pairmode 0.1.0 with `binding: version`
  (Signal-1 absent), confirmed via fresh `fleet_discovery.py` run 2026-07-21.
- **0.1.0 schema gap**: `/mnt/work/Repo-G`'s `.companion/state.json` is on
...
- **Cross-repo scope, not enforced by scope_guard**: this story's actual file
  writes happen entirely inside `/mnt/work/Repo-G`, outside
...
- Seam gate (`docs/harness-cutover-runbook.md` § Seam gate) passes for
  `/mnt/work/Repo-G`:
  - `git -C /mnt/work/Repo-G status --porcelain` is empty (all work committed
    and pushed).
  - `uv run python flex_build.py read-attempt-count` (run from
    `/mnt/work/Repo-G`) returns `0`.
  - `/mnt/work/Repo-G` is at a phase boundary (checkpoint tag, or every
    current-phase story `status: complete`).
  - `flex_build.py check-stubs --project-dir /mnt/work/Repo-G` returns clean.
  - `/mnt/work/Repo-G` appears in `fleet_discovery.py list-projects`.
  If any seam-gate item fails, defer this story until `/mnt/work/Repo-G`'s
  own maintainer resolves it — do not force migration mid-story.

## Ensures

- `/mnt/work/Repo-G`'s `.companion/state.json` reads
  `"pairmode_version": "0.3.0"`.
- `/mnt/work/Repo-G`'s `CLAUDE.build.md` bakes an absolute
  `pairmode_scripts_dir` pointing at
  `/mnt/work/flex-harness/skills/pairmode/scripts` (Signal-1 present).
- `fleet_discovery.py discover --project-dir /mnt/work/Repo-G` (run from
  flex-harness) reports `binding: scripts`.
- One complete pairmode story cycle has run through `/mnt/work/Repo-G`'s
  migrated loop with no regressions, and the result is committed and pushed
  in `/mnt/work/Repo-G`'s own repo.
...
§ Per-project mechanic) against `/mnt/work/Repo-G`, all commands run with
`PATH=$HOME/.local/bin:$PATH` from `/mnt/work/flex-harness` except where
noted:

1. Confirm the seam gate (Requires) passes.
2. Dry-run:
   `uv run python skills/pairmode/scripts/pairmode_sync.py sync-all --project-dir /mnt/work/Repo-G --dry-run`.
   Review the diff.
3. Apply:
   `uv run python skills/pairmode/scripts/pairmode_sync.py sync-all --project-dir /mnt/work/Repo-G --apply --yes`.
4. Normalize:
   `uv run python skills/pairmode/scripts/pairmode_migrate.py to-030 --project-dir /mnt/work/Repo-G --apply`.

4a. **0.1.0 backfill** (this project only): after step 4 and before step 5,
    diff `/mnt/work/Repo-G/.companion/state.json`'s keys against a known-good
    0.2.0 project (e.g. `/mnt/work/Repo-L`). Manually add any of
...
5. Verify Signal-1:
   `uv run python skills/pairmode/scripts/fleet_discovery.py discover --project-dir /mnt/work/Repo-G`
   shows `binding: scripts`. If absent, stop and fix `CLAUDE.build.md` before
   continuing.
6. Run one complete story cycle in `/mnt/work/Repo-G`'s migrated loop (any
   story — doc/lesson stories are fine). Confirm `pairmode_version: 0.3.0`
   in `.companion/state.json`, then from `/mnt/work/Repo-G`:
...
(`git -C /mnt/work/Repo-G checkout HEAD -- CLAUDE.build.md .companion/state.json`)
and re-run the Era 2 loop; do not leave `/mnt/work/Repo-G` half-migrated.

## Tests

- `PATH=$HOME/.local/bin:$PATH uv run python skills/pairmode/scripts/fleet_discovery.py discover --project-dir /mnt/work/Repo-G`
  reports `pairmode_version: 0.3.0` and `binding: scripts`.
- `git -C /mnt/work/Repo-G status --porcelain` is empty after the story
  (committed and pushed).
- If `/mnt/work/Repo-G` has its own test suite, it passes post-migration
