...
id: RELEASE-065
rail: RELEASE
title: Migrate Repo-C to pairmode 0.3.0 (seed never delivered)
status: complete
phase: "106"
auth_gated: false
...
<!-- SPEC-WRITER NOTE (frontmatter): the stub arrived with no `primary_files:` key,
     ...
     *this file* — the `## Evidence` section appended by the executor. Every other
     write target is outside the repo, under `/mnt/work/Repo-C`, and therefore cannot
...
For Repo-C specifically, that per-project seed prompt was never delivered at all —
so Repo-C has had *no* prior migration work of any kind, not even a partial or
abandoned attempt. The practical consequence is that **Repo-C's pre-state is not
known to this spec and must not be assumed.** Unlike Repo-J — whose
`binding: version / signal1 absent / signal2 0.2.0` baseline was already recorded
verbatim in RELEASE-063's E1 — Repo-C's binding, declared scripts path, recorded
...
recorded baseline is a **hard gate** on proceeding: no write to `/mnt/work/Repo-C`
happens before that output exists in `## Evidence`.

**The campaign gate is overridden, and this story carries the debt.**
RELEASE-064 completed the migration *mechanic* on Repo-J (E0–E4, E8–E14 pass) but
its proving cycle was **deferred by explicit operator decision** — recorded
verbatim in RELEASE-064 `## Evidence` § *E5 / E6 / E7*: *"Repo-J isn't in a spot to
build, we'll just have to mark it complete and continue on. I'll have to prove it
out later."* RELEASE-064's E10 additionally established that Repo-B has had **no
...
- If Repo-C **can** run a proving cycle, then this story's E5/E6/E7 become the
...
- If Repo-C **cannot** (no buildable work, operator unavailable for a native
...
- **CER-110 — fleet-wide plugin-sourced duplicate-hook signal.** Since cp-110's
  ...
  `security_reminder_hook.py` ×6 on Repo-J), unprunable by `audit-hooks --apply`
...
- **CER-111 — `to-030` silently rewrites custom `expected_step_tokens`.** On the
  canary a custom value was kept with a WARN; on Repo-J `to-030` silently rewrote
...
observed post-sync state of those names in Repo-C rather than assuming it.
...
(`state.json.lock` residue) **recurred on Repo-J** and their corrected forms are in
RELEASE-064's Evidence; note 7 (auto-mode classifier block) **did not recur**; and
note 9 (`settings.local.json` sediment) was **not applicable** to Repo-J, which has
no such file. Repo-C may differ on all of these — "did not recur once" is not
...
1. **No sandboxed builder subagent, no flex worktree.** The write targets live at
   `/mnt/work/Repo-C`, outside this repo. The standard worktree loop and
...
- **cp-111 is tagged and its content is present in the channel.** Phase 111
  ...
  that content into Repo-C, so the channel state at migration time is part of what
...
- **Repo-C exists as a git repository and its working tree is clean** at the moment
  the mechanic begins. The path is *expected* to be `/mnt/work/Repo-C`; confirm it
  from `fleet_discovery.py`'s candidate scan rather than assuming, and if Repo-C is
...
- The operator is present. Beyond the override confirmation, canary note 7's
  ...
  --apply` may or may not recur (it did on Repo-B, it did not on Repo-J), and the
...
**E1. A pre-migration baseline exists, and Repo-C's unknown pre-state is
characterized.**
`## Evidence` contains the verbatim output of a `fleet_discovery.py` run captured
**before any write to Repo-C**, showing Repo-C's pre-migration `binding`, `signal1`
(scripts path) and `signal2` (`pairmode_version`), plus the run's
`Projects with duplicate hooks:` line. `## Evidence` also records
`git -C <Repo-C> log --oneline -5` and `git -C <Repo-C> status --porcelain`, and
states in one line which starting shape Repo-C is in — bound-0.2.x, bound-other-
...
something" is unverifiable, and for Repo-C specifically there is no prior recorded
baseline anywhere in the campaign to fall back on.

**E2. Repo-C reports pairmode 0.3.0 and binds the release channel.**
A post-migration `fleet_discovery.py` run, recorded in `## Evidence` and using the
**same command form as E1** so the two are directly comparable, shows Repo-C with:
...
channel declaration did not, and Repo-C would still be consuming an unknown copy of
the scripts.

**E3. Repo-C's *pairmode* hooks are a single block per event (CER-110-aware).**
...
- (a) Repo-C's own hook state from its `.claude/settings.json`, showing **exactly
...
- (b) `pairmode_sync.py audit-hooks --project-dir <Repo-C>` (dry-run), showing that
...
- (c) the fleet-wide `Projects with duplicate hooks: N` line from the E2 run,
  ...
  is not expected to be `0` and is not a Repo-C migration defect.
...
treat it as a stop. Do not hand-edit Repo-C's settings to make any number come out
right — the point is to prove `pairmode_sync.py` produces this state on its own.

**E4. Repo-C's bootstrapped loop is the 0.3.0 thin-harness template, and the
cp-111 packaging content is recorded.**
`## Evidence` records:
- (a) the result of inspecting `<Repo-C>/CLAUDE.build.md` and confirms it is the
  ...
  `head -5`;
- (b) the observed state of any `SKILL.md` content `sync-all` wrote into Repo-C —
  ...
  surprise here is a finding for E12, not a reason to hand-edit Repo-C.

**E5. A proving story cycle completed inside Repo-C — or its absence is recorded
as an explicit, operator-owned deferral.**

*Preferred path (proof).* `## Evidence` names the Repo-C-side story ID built as the
proving cycle (mechanic step 6), states that it ran inside Repo-C's **own**
`CLAUDE.build.md` loop with Repo-C's own story numbering, states the full cycle it
traversed (spec-writer → builder → reviewer → merge), and states its outcome. The
story is a real, small, genuinely-wanted piece of Repo-C work — not a throwaway.
Per canary playbook note 8, the cycle runs in a **native Repo-C session** unless
...
*Fallback path (deferral).* If Repo-C cannot run a proving cycle, `## Evidence`
records, in the same shape RELEASE-064 used: the operator's decision **quoted
verbatim**; the reason Repo-C could not build; and an explicit statement that
...
across the whole campaign (Repo-B, Repo-J and Repo-C). A follow-up naming where the
proof will come from is filed under E13.

Silence is a fail. One of the two paths must be explicit in `## Evidence`.

**E6. The proving cycle's attempt rows landed in Repo-C's effort.db — correctly.**
...
from recorded queries against Repo-C's `effort.db` (locate it; do not assume the
path — Repo-B's and Repo-J's were `.companion/effort.db`). Verify the live schema
...
- **E6a — attribution (CER-103).** At least one attempt row exists for the E5
  story ID **in Repo-C's own db**, not flex's. `## Evidence` also records the
...
- **E6c — no duplicates (CER-104).** Each attempt appears **once**. Record a
  ...
  incidentally confirmed the historical failure shape on Repo-B (rows 281/282:
...
**E7. Repo-C's checkpoint/report path sees the attempts.**
Applies on the same condition as E6. `## Evidence` records the output of the
pairmode checkpoint or attempt-report CLI run against Repo-C for the phase
...
**E8. Repo-C's git history shows the migration as its own commit(s).**
`## Evidence` records `git -C <Repo-C> log --oneline` covering the migration
commit(s) and, if E5 ran, the proving-story commit(s), so a later auditor can see
exactly what the sync wrote into Repo-C and roll it back if needed. Per canary
playbook note 3 — which recurred on Repo-J — the migration commit **precedes** the
proving cycle in history. Record the file/insertion/deletion counts for the
migration commit.

**E9. Repo-C's `settings.local.json` sediment is handled deliberately.**
Per canary playbook note 9 (Repo-B carried 133 accumulated allow rules, 91 of them
stale `Write(path)`/per-file `Edit(path)` entries that flood the first
post-migration session with warnings; Repo-J had no such file at all), `## Evidence`
records: whether `<Repo-C>/.claude/settings.local.json` exists; if it does, the
...
restated.**
RELEASE-064 E10 established that Repo-B had no post-cp-110 rows and that Repo-J's
...
- **(a)** fresh post-cp-110 attempt rows now exist somewhere in the fleet
  (Repo-B's next cycle, Repo-J's deferred cycle, or Repo-C's own E6) — record the
...
RELEASE-064 already determined that **Repo-B does not need a re-sync** to pick up
cp-110 (the fixes live in channel scripts Repo-B invokes by path). Do not re-derive
...
recur**, or **was not applicable** on Repo-C — and then lists any *further* new
...
- **note 7** (auto-mode classifier block) — recurred on Repo-B, did not recur on
  Repo-J. State which happened here; two runs disagreeing means one data point does
  not settle it.
- **note 6 / CER-111** (`expected_step_tokens`) — Repo-B kept its custom value
  with a WARN; Repo-J's was silently rewritten `53000 → 5000`. `## Evidence` must
  record Repo-C's **pre-`to-030` value**, the **post-`to-030` value**, and whether a
...
worktree. Do not attempt to have a builder subagent write to `/mnt/work/Repo-C` —
...
2. **Confirm the remaining preconditions before touching anything.** Verify every
   ...
   present; RELEASE-066..071 still `draft`; Repo-C located and its working tree
...
4. **Capture the baseline and classify Repo-C's starting shape (E1).** Run
   `fleet_discovery.py` from the release channel. Per canary playbook note 2 —
   which recurred on Repo-J — the runbook's step-5 command form is **wrong** (it
   ...
   PATH=$HOME/.local/bin:$PATH uv run python \
     ...
     --candidate-dir /mnt/work/Repo-C --no-snapshot
   ```
   ...
   against the post-migration run. Also capture `git -C <Repo-C> log --oneline -5`
   and `git -C <Repo-C> status --porcelain`.

   Then **state Repo-C's starting shape in one line** and pick the step-5 branch.
   Because Repo-C's seed was never delivered, do not assume Repo-J's shape.
...
6. **Run the mechanic against Repo-C.** Follow the runbook's six steps in order,
   ...
   - **note 7 (unsettled):** the auto-mode permission classifier blocked the first
     out-of-repo `sync-all --apply` on Repo-B but not on Repo-J. If it blocks, ask
   ...
   - **CER-111 — mandatory pre-read:** **before** running `to-030 --apply`, read
     and record Repo-C's current `expected_step_tokens` value from its state file.
   ...
   step fails, **stop** — do not improvise a fix into Repo-C. Report to the
...
7. **Commit the migration before the proving cycle (canary note 3, recurred on
   Repo-J).** The runbook orders the commit after step 6; that is wrong for the
   0.3.0 loop, because the proving story's worktree snapshots git HEAD and would
   not see the migration. Commit the sync/migration changes into Repo-C as their
...
8. **Handle the `settings.local.json` sediment (E9, canary note 9).** Before
   handing Repo-C to a native session, check whether
   `<Repo-C>/.claude/settings.local.json` exists and count the stale
   ...
   that (as Repo-J's did not).

9. **Verify the stamp before proving (E2, E3, E4).** Re-run the exact step-4
   `fleet_discovery.py` command and confirm Repo-C now reports `0.3.0`,
   `binding: both` with `signal1` pointing at the channel. Then run
   `audit-hooks --project-dir <Repo-C>` as a dry-run and inspect Repo-C's
   ...
   `<Repo-C>/CLAUDE.build.md` for the thin-harness template and record the cp-111
...
10. **Run the proving story cycle (E5, E6, E7) — this is the most valuable output
    ...
    deferred and Repo-B has produced no post-cp-110 rows, **a passing E6 here is
    ...
    Default to a **native Repo-C session**, in Repo-C's own `CLAUDE.build.md` loop,
    with Repo-C's own story numbering; if the operator instead wants to exercise the
    ...
    already-wanted piece of Repo-C work — a no-op story defeats the purpose.
    ...
    If Repo-C genuinely cannot run a cycle, take E5's fallback path — record the
...
11. **Record the evidence (E0–E14).** Append a `## Evidence` section to *this
    ...
    E6a/E6b/E6c queries and output; the E7 report output; the E8 Repo-C git log; the
...
13. **Ideology note (Step 4a — resolved inline, no conflict).** Four things in
    ...
    coarse after-the-fact check would have silently passed on Repo-J's rewrite.
    *"Rationale-bearing decisions over bare rules"* is why E3 was **restated**
    ...
    convenience"* is why E1 makes Repo-C's unknown pre-state a hard gate with a
    branch instead of presuming Repo-J's shape. On accepted constraints: *"Hooks are
    thin relays only"* is adjacent, since the mechanic rewrites Repo-C's hook block
    — the rationale is that hooks must not block or write state, so E3 forbids
    hand-editing Repo-C's settings to satisfy an assertion and step 6 forbids routing
...
```bash
# E1/E2/E3 — Repo-C baseline and post-migration state (confirm flags via --help first;
# the runbook's step-5 form is wrong per RELEASE-063 note 2, recurred on Repo-J)
PATH=$HOME/.local/bin:$PATH uv run python \
  /mnt/work/flex-harness/skills/pairmode/scripts/fleet_discovery.py \
  --candidate-dir /mnt/work/Repo-C --no-snapshot
```
...
PATH=$HOME/.local/bin:$PATH uv run python \
  ...
  audit-hooks --project-dir /mnt/work/Repo-C
```

```bash
# E4 — thin-harness template, and cp-111 bare skill names as landed in the target
grep -c "flex_build.py next-action" /mnt/work/Repo-C/CLAUDE.build.md
head -5 /mnt/work/Repo-C/CLAUDE.build.md
grep -rn '^name:' /mnt/work/Repo-C/.claude/skills/*/SKILL.md 2>/dev/null || \
  echo "no SKILL.md copied into target — record this"
```

```bash
# CER-111 — expected_step_tokens, BEFORE and AFTER to-030
grep -n 'expected_step_tokens' /mnt/work/Repo-C/.companion/state.json
# ... run to-030 --apply ...
grep -n 'expected_step_tokens' /mnt/work/Repo-C/.companion/state.json
```

```bash
# E6 — proving-cycle attempt rows in Repo-C's own effort.db.
...
find /mnt/work/Repo-C -name 'effort.db' -not -path '*/node_modules/*'
sqlite3 <Repo-C-effort.db> ".schema attempts"

# E6a attribution: rows present in Repo-C, absent in flex
sqlite3 <Repo-C-effort.db> "SELECT * FROM attempts WHERE story_id='<E5-STORY-ID>'"
...
```bash
# E7 — Repo-C's checkpoint/attempt report sees the rows (discover the exact
...
ls /mnt/work/Repo-C/.claude/settings.local.json 2>/dev/null || echo "no settings.local.json"
grep -c 'Write(' /mnt/work/Repo-C/.claude/settings.local.json
grep -c 'Edit('  /mnt/work/Repo-C/.claude/settings.local.json
```
...
```bash
# E8 — migration visible in Repo-C's history, migration commit before proving commit
git -C /mnt/work/Repo-C log --oneline -10
...
- Repo-C shows `binding: both` and `0.3.0`, with single-block **pairmode** hooks per
...
- either (i) E5/E6/E7 pass — rows in Repo-C's own db, populated `outcome`/tokens,
...
It also references `/mnt/work/Repo-C`, `/mnt/work/Repo-B`,
...
context supplied with this story. All concrete paths inside Repo-C
(`.companion/state.json`, `.claude/settings.local.json`, `.claude/skills/*/SKILL.md`,
the `effort.db` location) are **expected** shapes taken from Repo-B and Repo-J, not
verified for Repo-C — locate each before using it, and record what you actually find.
...
- **Migrating any project other than Repo-C.** Repo-D, Repo-F, Repo-K,
  base56 and Repo-G are RELEASE-066..070. Do not run the mechanic against a second
  project "while the environment is warm" — the campaign is running on an
  overridden gate and wants each data point separable.
- **Re-syncing or re-migrating Repo-B or Repo-J.** E10 requires *re-checking* the
  proof debt and recording the position. Performing a re-sync, or running Repo-J's
...
- **The full-fleet DP8 gate and the phase-97 close.** Both are RELEASE-071 (phase
  ...
  snapshot.
- **Building Repo-C's proving story to a flex-side spec.** The proving cycle is
  Repo-C's own work, in Repo-C's numbering, under Repo-C's loop. It gets no flex story
...
`/mnt/work/Repo-C`. **Mixed result:** migration mechanic and stamp complete
...
**Dirty-tree stop condition (canary note 1) fired.** Repo-C's tree carried a
...
scaffold left in history (later resolved by Repo-C's own session as
"PAIRMODE-001 complete — migration applied externally").

```
$ fleet_discovery.py --candidate-dir /mnt/work/Repo-C --no-snapshot   [channel scripts]
  /mnt/work/Repo-C
    binding: version
    signal1 (scripts path): absent — no-declaration
    signal2 (pairmode_version): 0.2.0
    DUPLICATE HOOKS: /mnt/work/Repo-C — events: SessionStart, PostToolUse
Projects with duplicate hooks: 16
```

**Starting shape: bound 0.2.x consumer** (branch 1 of Instructions step 5 — the
ordinary path; same shape as Repo-J). Pre-state git:

```
$ git -C /mnt/work/Repo-C log --oneline -3
...
$ git -C /mnt/work/Repo-C status --porcelain   (after operator-approved discard)
(no output — clean)
```
...
- Dry-run reviewed by operator (1387 lines; 5 agent re-renders +
  `CLAUDE.build.md` thin-harness rewrite — Repo-J's shape exactly); approved.
...
- **CER-111 watch:** `expected_step_tokens` pre-read **53416**; post-to-030
  **53416** — `[WARN] custom expected_step_tokens=53416 — value kept (not the
  Era 2 stamp).` **Kept with WARN**, matching Repo-B; isolates Repo-J's silent
...
- Migration committed **before** the proving cycle (note 3 applied):
  Repo-C `909ef3b` — `sync: migrate to pairmode 0.3.0 thin-harness loop` —
...
`security_reminder_hook.py` ×6 — identical to Repo-J/fleet, CER-110). Fleet-wide
...
$ grep -c "flex_build.py next-action" /mnt/work/Repo-C/CLAUDE.build.md
2
# CLAUDE.build.md — Repo-C Build Orchestrator
```

E4(b): Repo-C's agent frontmatter names are bare (`builder`, `reviewer`, …);
Repo-C has no `skills/` dir, so cp-111's SKILL.md renames have no downstream
copy here — RELEASE-064's "Repo-B needs no re-sync" determination unchanged
under cp-111.

### E5 — proving cycle (native)

Session mode: **native Repo-C session** (operator-run, Repo-C's own
...
and pushed. Repo-C-side friction the operator's session logged (their CER-C001..
...
`effort_recording.log` shows `target_project: /mnt/work/Repo-C` on every row —
...
commit a partial row (CER-091 defect-2 branch). Root cause: Repo-C's workers
...
sections but **preserves stale body content**: Repo-C's `builder.md:106` still
...
refused external reconciliation while the Repo-C session was live (its
...
```
$ flex_build.py checkpoint-report --project-dir /mnt/work/Repo-C
...
The lifetime rollup sees Repo-C's attempts (no "no attempts recorded"); the
phase-scoped half cannot resolve because Repo-C's phase (EH005-main) was already
...
`/mnt/work/Repo-C/.claude/settings.local.json.bak-pre-030-prune`, 48 rules
removed, 13 retained; post-prune `Write(`/`Edit(` counts both 0.

### E10 — proof-debt re-check

Repo-B: still no post-cp-110 rows at execution time. **This story's E6a/E6c
...
Repo-C's native proving session ran `fleet_discovery.py` without
...
4. Agent-cleanup WARN: **recurred (3-of-3) — RECLASSIFIED: not noise.** The
   warn flags stale 0.2-era body content in synced agent files; on Repo-C that
...
6. `expected_step_tokens`: Repo-C **kept 53416 with WARN** (Repo-B-like);
   three-way comparison shows Repo-J's silent rewrite is value-dependent
...
9. Sediment: **recurred** (2-of-3; Repo-J n/a) — 48 rules pruned with backup.

New findings this run:

- **(new-1) Default snapshot write pollutes the channel checkout.**
  `fleet_discovery.py` run without `--no-snapshot` (by Repo-C's native session)
...
- **(new-3) Repo-C's own session filed CER-C001..C004 in Repo-C's backlog**
...
- **CER to file: `_check_cer_do_now` reads the scaffolded `(none)` placeholder
  row as an unresolved Do-Now item** (new-3; Repo-C CER-C004) — blocks every
...
- **CER-111 update**: three-way `expected_step_tokens` data recorded here;
  Repo-J's rewrite is value-dependent.
- **Repo-B post-cp-110 E6 re-verification**: still outstanding (now with the
  caveat that Repo-B's agent bodies predate the JSON grammar too — its next
...
(CER-104)** — supplied by Repo-C story PAIRMODE-002. **CER-101's content half
