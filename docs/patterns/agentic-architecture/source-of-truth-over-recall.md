...
|-------------|---------|-------|
| flex project, operated by david@halfhorse.com | `context_budget.py` re-reads `.companion/state.json` on every invocation rather than caching the threshold in context. The reviewer sub-agent reads the story file by ID — not from a value copied from the orchestrator's context. The orchestrator re-reads each story's `status` field before each loop iteration, preventing completion-by-recall. These implementations were formalized after three distinct stale-recall incidents: a blocked story spawn due to a compacted threshold value (Phase 47), an architecture doc carrying a stale migration row count found by cold-eyes review (CER-015, Repo-E), and a story incorrectly reported complete after context compaction (Phase 46). The same canonical-read discipline applies to template synchronization in downstream projects (Repo-G, Repo-I — Phase 47 T8 recon), where stale `CLAUDE.md` content in two projects simultaneously confirmed the failure is systemic rather than incidental. | 1 primary repo + 5 downstream projects (Repo-E, Repo-L, Repo-I, Repo-H, Repo-G) |
