...
- **Fleet verification (INFRA-209)** — 13 of 14 in-scope fleet projects already carried the three context-budget-gate registrations by the time of verification (read-only audit, no commits needed); `Repo-G` formally excluded as a known carve-out; `anchor` remains excluded as a non-pairmode-consumer sibling plugin repo; `Repo-I`'s forged CER-067 workaround keys in `state.json` are still present, reset deferred as a follow-up (out of scope for this phase).
