...
| ID | Title | Status |
|----|-------|--------|
| OBS-001 | Resolver state-model read API (`resolver-state --json` + TS reader) | planned |
| OBS-002 | SPA UI refactor to the resolver state model | planned |
| OBS-003 | D1 display half — `expected_step_tokens` provenance (CER-053) | planned |
| OBS-004 | D2 — `context_current_tokens` live writer root-cause + fix (CER-054) | planned |
| OBS-005 | D3 — waypoint outcome recording + render (CER-055) | planned |
| INFRA-164 | `flex_observability.py` CLI hardening (re-homed from Phase 64) | planned |
