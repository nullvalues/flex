...
| ID | Title | Status |
|----|-------|--------|
| RELEASE-063 | Migrate Repo-B to pairmode 0.3.0 (campaign canary) | complete |
| RELEASE-064 | Migrate Repo-J to pairmode 0.3.0 | complete |
| RELEASE-065 | Migrate Repo-C to pairmode 0.3.0 (seed never delivered) | complete |
| RELEASE-066 | Migrate Repo-D to pairmode 0.3.0 | complete |
| RELEASE-067 | Migrate Repo-F to pairmode 0.3.0 | complete |
| RELEASE-068 | Migrate Repo-K to pairmode 0.3.0 — canon files only | complete |
| RELEASE-069 | Decommission pairmode from base56 (strip, not migrate — product is fully developed) | complete |
| RELEASE-070 | Migrate Repo-G from 0.1.0 to 0.3.0 (unpark RELEASE-030 lesson-extraction carve-out) — hand-migrated outside this campaign, carve-out unverified, see § Evidence | complete |
...
RELEASE-063 (Repo-B, canary) strictly first — it proves the campaign playbook.
RELEASE-064..067 in any order after the canary passes. RELEASE-068 (Repo-K — narrowed
...
at the end of this doc), RELEASE-070 (Repo-G, originally scoped as a 0.1.0 migration gap
...
(binding: both, 0.3.0), **1/15 partial** (Repo-G — `binding: version` only, hooks resolve
...
RELEASE-063 (canary) executed 2026-07-27/28: Repo-B migrated and verified
...
RELEASE-064 (Repo-J) then ran the mechanic successfully but its proving cycle
...
**Re-blocked (2026-07-28, RELEASE-065 verdict).** The Repo-C migration's E6
...
explicit reconcile sweep against Repo-C's live `effort.db` turned rows 33/34
...
**canon-only** migration of Repo-K: sync the 0.3.0 canon surfaces
(`CLAUDE.md`, `CLAUDE.build.md`, the seven `.claude/agents/` shells,
`.companion/state.json`) into `/mnt/work/Repo-K` and nothing more, so this phase can
close. The original "coordinate around in-flight UAT-gated work" scope is
**superseded** — the coordination risk is eliminated by not touching Repo-K's code,
docs, stories, backlog or UAT work rather than by scheduling around them. No
proving story cycle runs for Repo-K, so the § *Checkpoint proves* per-project
proving clause is narrowed for Repo-K by this directive and RELEASE-071 must count
it as **proof-deferred, not proven** (the campaign's downstream proof already
stands on Repo-D/Repo-F).
